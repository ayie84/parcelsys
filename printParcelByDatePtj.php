...
con2db();
$dateGlobal = trim($_REQUEST['date']);

//echo $date;

$ptj=trim($_REQUEST['ptj']);
...
echo "string";
	echo "SELECT ptj_pic FROM  `ptj` where `ptj_name` LIKE '%".$ptj."%'";

//dapatkan $ptj_pic
$result = mysql_query("SELECT ptj_pic FROM  `ptj` where `ptj_name` LIKE '%".$ptj."%'") or die (mysql_query());

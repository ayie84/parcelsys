	   ...
	   <!-- DropDown Menu For Parcel Start -->
        ...
        <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Parcel<span class="caret"></span></a>
			<ul class="dropdown-menu">
				...
				<li><a href="parcelExpress.php">Register Express</a></li>
...
function courierDropMenu($courier = "",$r = "")
{
	con2db();
	...
	echo "<select name='courier' id='courier' class='form-control'".$r.">";
			...
			$totalList =  mysql_query("SELECT COUNT(parcel_courier) AS parceltotal FROM parcel WHERE parcel_timestamp LIKE '%".$date." %'") or die (mysql_query());
					...
					echo '<div class="row">';
						echo '<div class="col-md-4""><h3 class="" style="margin-top:0;margin-bottom:0;"> '.$tableTitle.' ('.mysql_result($totalList, 0).') </h3></div>';
			  			...
			  			<p><a href="parcelByPtj.php" class="btn btn-info">New Parcel</a> 
			...
			<nav aria-label="Page navigation">
			  <ul class="pagination">';
			   ...
			   for ($i=1; $i<=$total_pages; $i++) {
							...
							echo '<li><a href="parcelByPtj.php?page='.$i.'">'.$i.'</a></li>';
							};
			...
			$totalList =  mysql_query("SELECT COUNT(parcel_courier) AS parceltotal FROM parcel WHERE parcel_timestamp LIKE '%".$date." %'") or die (mysql_query());
					...
					echo '<div class="row">';
						echo '<div class="col-md-4""><h3 class="" style="margin-top:0;margin-bottom:0;"> '.$tableTitle.' ('.mysql_result($totalList, 0).') </h3></div>';
			  			...
			  			<p><a href="parcelExpress.php" class="btn btn-info">New Parcel</a> 
			...
			<nav aria-label="Page navigation">
			  <ul class="pagination">';
			   ...
			   for ($i=1; $i<=$total_pages; $i++) {
							...
							echo '<li><a href="parcelExpress.php?page='.$i.'">'.$i.'</a></li>';
							};
			...
			$totalList =  mysql_query("SELECT COUNT(parcel_courier) AS parceltotal FROM parcel WHERE parcel_timestamp LIKE '%".$date." %'") or die (mysql_query());
					...
					echo '<div class="row">';
						echo '<div class="col-md-4""><h3 class="" style="margin-top:0;margin-bottom:0;"> '.$tableTitle.' ('.mysql_result($totalList, 0).') </h3></div>';
			  			...
			  			<!-- <p><a href="parcelViewToday.php" class="btn btn-info">New Parcel</a> -->
...
function parcelViewTodayKK()
{

	date_default_timezone_set("Asia/Kuala_Lumpur");
    //echo date('d-m-Y H:i:s'); //Returns IST
   // echo date('Y-m-d H:i:s'); //Returns IST
	$tableTitle = 'Received Parcel Today Student (KK)';
	$today = date('Y-m-d H:i:s');
	//echo $today;
	//$today='2016-11-13 15:28:10';
...
function parcelViewTodayStaff()
{

	date_default_timezone_set("Asia/Kuala_Lumpur");
    //echo date('d-m-Y H:i:s'); //Returns IST
   // echo date('Y-m-d H:i:s'); //Returns IST
	$tableTitle = 'Received Parcel Today Staff';
	$today = date('Y-m-d H:i:s');
	//echo $today;
	//$today='2016-11-13 15:28:10';
			...
			$totalList =  mysql_query("SELECT COUNT(parcel_courier) AS parceltotal FROM parcel WHERE parcel_timestamp LIKE '%".$date." %' AND parcel_ptj NOT Like '%KK1%' AND parcel_ptj NOT Like '%KK2%' AND parcel_ptj NOT Like '%KK3%' AND parcel_ptj NOT Like '%KK4%' AND parcel_ptj NOT Like '%KK5%'") or die (mysql_query());
					...
					echo '<div class="row">';
						echo '<div class="col-md-4""><h3 class="" style="margin-top:0;margin-bottom:0;"> '.$tableTitle.' ('.mysql_result($totalList, 0).') </h3></div>';
			  			...
			  			<a target = "_blank" href="printParcelStaff.php?date='.$date.'&ptj=" class="btn btn-warning" >Export To PDF</a></p>
...
function parcelViewTodayStaffCourier()
{
   ...
   // echo date('Y-m-d H:i:s'); //Returns IST
	...
	//echo  "SELECT COUNT( * ) AS Value FROM `parcel` where `parcel_timestamp` LIKE '%"2016-12-16%' AND parcel_ptj NOT Like '%KK1%' AND parcel_ptj NOT Like '%KK2%' AND parcel_ptj NOT Like '%KK3%' AND parcel_ptj NOT Like '%KK4%' AND parcel_ptj NOT Like '%KK5%'";



	//$value = mysql_query("SELECT COUNT( * ) AS Value FROM  `parcel` where `parcel_timestamp` LIKE '%2016%'") or die (mysql_query());

//Select * from ptj where ptj_name NOT Like '%KK1%' AND ptj_name NOT Like '%KK2%' AND ptj_name NOT Like '%KK3%' AND ptj_name NOT Like '%KK4%' AND ptj_name NOT Like '%KK5%'
